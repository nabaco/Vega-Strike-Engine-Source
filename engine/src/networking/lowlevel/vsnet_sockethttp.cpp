...
VsnetHTTPSocket::VsnetHTTPSocket( const std::string &uri, SocketSet &set ) :
    VsnetSocketBase( -1, "http", set )
    , _incompleteheadersection( 0 )
    , _content_length( 0 )
    , _send_more_data( false )
...
bool VsnetHTTPSocket::lower_selected( int datalen )
{
    ...
    while (datalen != 0) {
        ...
        if (ret == 0) {
            ...
            if ( _content_length >= 0
                && ( (readHeader == false /*set to false on success return*/ && dataToReceive.length()
                      == 0) || readHeader == true ) ) {
                //incomplete transfer...
...
donewiththis:
    ...
    if (_content_length == 0 && !_send_more_data && _fd != -1) {
        ...
        this->_fd = -1;
        //reopenConnection();
        if ( dataToReceive.length() == 0 || (dataToReceive[0] == '!' && dataToReceive.length() < 4) ) {
            //don't bother with the bangs
            dataToReceive    = std::string();
